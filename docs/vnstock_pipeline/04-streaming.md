# Vnstock Pipeline - Dữ Liệu Thời Gian Thực

## Giới Thiệu

Chương này giới thiệu tổng quan tính năng **truyền phát thời gian thực** (streaming) của `vnstock_pipeline` nhằm:

- Nhận cập nhật giá thời gian thực từ WebSocket mà không phải gọi API liên tục gây vượt giới hạn kết nối.
- Quản lý phiên giao dịch tự động (ví dụ: nghỉ trưa, kết nối lại).
- Tự động phân giải các mã phái sinh sang định dạng chuẩn KRX.
- Xử lý dữ liệu luồng mạnh mẽ qua kiến trúc Processors.
...
Vì Streaming là một tính năng phức tạp đòi hỏi tổ chức mã nguồn tốt, tài liệu này đi kèm với một bộ mã nguồn minh họa rất chi tiết. **Bộ code này không được chia sẻ công khai trên GitHub.**

> [!TIP]
>
> **Tải bộ code mẫu:** Vui lòng đăng nhập vào tài khoản trên Vnstocks và truy cập link sau:
> [Tải xuống tại mục Tài nguyên độc quyền](https://vnstocks.com/account?section=packages&tab=exclusive-files)
>
...
- **`02_custom_processor.py`**: Minh họa cách kế thừa class `DataProcessor` của Vnstock, cho phép bạn tự định nghĩa logic lọc hoặc lưu trữ dữ liệu (lưu CSV, Parquet) khi có tín hiệu mới đến.
...
- **`01_publisher.py`**: Thiết lập một `RedisPubSubProcessor`. Nhiệm vụ duy nhất của nó là nhận dữ liệu thô và đẩy vào bộ nhớ Redis.
...
- **Quét đột biến**: Các file `01_rsi_alert.py`, `02_macd_crossover.py`, và `03_volume_spike.py` chứa logic cấu trúc bộ đệm giá để tính toán các tín hiệu mua/bán trong thời gian thực.
- **Gửi thông báo**: Script `04_discord_webhook_alert.py` hướng dẫn cách bọc các lời gọi mạng vào các `asyncio.Task` phi đồng bộ, giúp bạn gửi tin nhắn cảnh báo lên kênh Discord mà không làm nghẽn luồng đọc giá chính.

---

## Lưu Ý Quan Trọng Khi Phát Triển Hệ Thống Streaming

1. **Khống Chế Bộ Nhớ**: Khi tính toán chỉ báo kỹ thuật thời gian thực, luôn sử dụng các cấu trúc dữ liệu có giới hạn vòng (như `collections.deque(maxlen=N)` trong Python). Tuyệt đối không dùng mảng List tiêu chuẩn, vì điều này sẽ gây tràn bộ nhớ RAM (OOM) sau nhiều giờ chạy liên tục trên VPS.
2. **Luồng Không Đợi**: Mọi tác vụ tốn thời gian như ghi vào Database ổ cứng hay gọi API mạng Webhook bên trong hàm `process()` đều phải được thực thi dưới dạng Async Tasks (`asyncio.create_task()`). Việc để block luồng thực thi chính dù chỉ vài phần mười giây cũng có thể khiến WSSClient rớt kết nối.